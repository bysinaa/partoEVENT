...
export class PagesService {
  ...
  async findAll({ page = 1, limit = 20, search, status }: { page: number; limit: number; search?: string; status?: string }) {
    ...
    const [items, total] = await Promise.all([
      this.prisma.page.findMany({ where, orderBy: { createdAt: 'desc' }, skip: (page - 1) * limit, take: limit }),
  ...
  async findOne(id: string) {
    const item = await this.prisma.page.findUnique({ where: { id } });
  ...
  async create(dto: any) {
    ...
    const existing = slug ? await this.prisma.page.findUnique({ where: { slug } }) : null;
    if (existing) throw new ConflictException('Slug already exists');
    return this.prisma.page.create({ data: { ...this.mapDto(dto), ...(slug && { slug }) } });
  }

  async update(id: string, dto: any) {
    await this.findOne(id);
    return this.prisma.page.update({ where: { id }, data: this.mapDto(dto) });
  }
